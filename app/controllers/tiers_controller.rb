class TiersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @tier.save
        format.html { redirect_to tier_url(@tier), notice: "Tier was successfully created." }
  ...
  def update
    respond_to do |format|
      if @tier.update(tier_params)
        format.html { redirect_to tier_url(@tier), notice: "Tier was successfully updated." }
  ...
  private
    ...
    def tier_params
      params.require(:tier).permit(:name, :likes, :max_file_size, :pets, :ads, :has_unlimited_likes)
    end
end
